...
//Correlation comment - Do not change!Original value='tid=3' Name ='CorrelationParameter_1'
	...
	web_url("forum.php_4",
		"URL=http://192.168.182.88/forum.php?mod=post&action=reply&fid=2&{CorrelationParameter_1}&infloat=yes&handlekey=reply&inajax=1&ajaxtarget=fwin_content_reply",
		"Resource=0",
...
*/
	...
	lr_start_transaction("resoponse");
	...
	web_submit_data("forum.php_6",
		...
		"Name=message", "Value={username}post{postid}-response{reponseid}", ENDITEM,
		LAST);
	...
	lr_end_transaction("resoponse", LR_AUTO);
